...
async function getServerTime() {
    ...
    try {
        // Supabase یه تابع now() داره که زمان سرور (UTC) رو برمی‌گردونه
...
// تابع کمکی: آیا زمان مسابقه گذشته؟ (با زمان سرور)
async function isMatchTimeExpired(match_date, match_time) {
    if (!match_date || !match_time) return false;
    const matchDateTime = new Date(`${match_date}T${match_time}`);
    const serverNow = await getServerTime();
    return serverNow >= matchDateTime;
}
            ...
            users.forEach(u => {
                ...
                row.innerHTML = `
                    <div class="user-info" style="width: 100%; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <span class="u-pass">🕒 ${visitTime}</span>
                    </div>
                `;
...
function initAdminFeatures() {
    ...
    window.editUserPass = async (id, name, pass) => {
        const n = prompt(`رمز جدید ${name}:`, pass);
        if (n) {
            await supabaseClient.from('project_users').update({ password: n }).eq('id', id);
            fetchUsers();
        }
    };
...
async function fetchAndRenderContent() {
    ...
    try {
        ...
        // یک‌بار زمان سرور رو می‌گیریم برای همه بازی‌های این رندر
        const serverNow = await getServerTime();
        ...
        folders.forEach(folder => {
            ...
            folderMatches.forEach(match => {
                ...
                if (match.match_date && match.match_time) {
                    ...
                    if (serverNow >= matchDateTime) { isTimeExpired = true; }
                }
